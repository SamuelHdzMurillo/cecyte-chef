...
const EventsTable = () => {
  ...
  const filteredEvents = events.filter(event => {
    ...
    const matchesStatus = !statusFilter || event.estatus_evento === statusFilter
  ...
  const sortedEvents = [...filteredEvents].sort((a, b) => {
    ...
    if (sortField === 'inicio_evento' || sortField === 'fin_evento' || sortField === 'created_at') {
  ...
  const validateForm = () => {
    ...
    if (formData.inicio_evento && formData.fin_evento) {
      if (new Date(formData.inicio_evento) >= new Date(formData.fin_evento)) {
        errors.fin_evento = 'La fecha de fin debe ser posterior a la fecha de inicio'
      }
    }
    ...
    if (!formData.lim_de_participantes_evento || formData.lim_de_participantes_evento < 1) {
      errors.lim_de_participantes_evento = 'El límite de participantes debe ser mayor a 0'
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (dataToSend.inicio_evento) {
        dataToSend.inicio_evento = new Date(dataToSend.inicio_evento).toISOString()
      }
      ...
      if (response.success || response.data) {
        ...
        closeModal()
        alert(isCreating ? 'Evento creado exitosamente' : 'Evento actualizado exitosamente')
      }
    } catch (error) {
  ...
  const handleDeleteEvent = async (event) => {
    if (window.confirm(`¿Estás seguro de que quieres eliminar el evento "${event.nombre_evento}"?`)) {
      try {
        setLoading(true)
        const token = authService.getToken()
  ...
  const activeEvents = events.filter(e => e.estatus_evento === 'activo').length
  const totalTeams = events.reduce((sum, event) => sum + (event.equipos?.length || 0), 0)
  const totalParticipants = events.reduce((sum, event) => sum + (event.lim_de_participantes_evento || 0), 0)
  ...
  return (
    <div className="events-table">
            ...
            <table className="table table-hover">
              <thead className="table-light">
                <tr>
                   ...
                   <th scope="col" style={{ width: '80px' }}>Equipos</th>
                ...
                {currentEvents.length === 0 ? (
                  <tr>
                    <td colSpan="9" className="text-center py-5">
                      ...
                      <p className="text-muted mb-3">No se encontraron eventos con los filtros actuales</p>
                      <button 
                      ...
                      <td className="text-center">
                        <span className="badge bg-secondary">{event.id}</span>
                         ...
                         <div className="d-flex align-items-center">
                           ...
                           <div>
                             <div className="fw-bold text-truncate" style={{ maxWidth: '180px' }} title={event.nombre_evento}>
                               {event.nombre_evento}
                             </div>
                           </div>
                         </div>
                       ...
                       <td>
                         <div className="event-dates">
                           ...
                           <small className="text-muted">{formatTime(event.inicio_evento)}</small>
                         </div>
                       </td>
                       <td>
                         <div className="event-dates">
                           <div>{formatDate(event.fin_evento)}</div>
                           <small className="text-muted">{formatTime(event.fin_evento)}</small>
                         </div>
                       </td>
                       <td>
                         <div className="event-location text-truncate" style={{ maxWidth: '130px' }} title={event.sede_evento}>
                       ...
                       <td className="text-center">
                         <span className={`badge bg-${getStatusBadgeColor(event.estatus_evento)} event-status-badge`}>
                ...
                <div className="modal-body">
                  <div className="row">
                    <div className="col-md-12 mb-3">
                      ...
                      <input
                        type="text"
                        className={`form-control ${formErrors.nombre_evento ? 'is-invalid' : ''}`}
                        id="nombre_evento"
                        name="nombre_evento"
                      ...
                      {formErrors.nombre_evento && (
                        <div className="invalid-feedback">{formErrors.nombre_evento}</div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="row">
                    <div className="col-md-6 mb-3">
                      ...
                      <input
                        type="date"
                        className={`form-control ${formErrors.inicio_evento ? 'is-invalid' : ''}`}
                        id="inicio_evento"
                        name="inicio_evento"
                      ...
                      {formErrors.inicio_evento && (
                        <div className="invalid-feedback">{formErrors.inicio_evento}</div>
                      )}
                    </div>
                    ...
                    <div className="col-md-6 mb-3">
                      ...
                      <input
                        type="date"
                        className={`form-control ${formErrors.fin_evento ? 'is-invalid' : ''}`}
                        id="fin_evento"
                        name="fin_evento"
                      ...
                      {formErrors.fin_evento && (
                        <div className="invalid-feedback">{formErrors.fin_evento}</div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="row">
                    <div className="col-md-8 mb-3">
                      ...
                      <input
                        type="text"
                        className={`form-control ${formErrors.sede_evento ? 'is-invalid' : ''}`}
                        id="sede_evento"
                        name="sede_evento"
                      ...
                      {formErrors.sede_evento && (
                        <div className="invalid-feedback">{formErrors.sede_evento}</div>
                      )}
                    </div>
                    
                    <div className="col-md-4 mb-3">
                      <label htmlFor="lim_de_participantes_evento" className="form-label">
                        Límite de Participantes <span className="text-danger">*</span>
                      </label>
                      <input
                        type="number"
                        className={`form-control ${formErrors.lim_de_participantes_evento ? 'is-invalid' : ''}`}
                      ...
                      {formErrors.lim_de_participantes_evento && (
                        <div className="invalid-feedback">{formErrors.lim_de_participantes_evento}</div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="row">
                    <div className="col-md-6 mb-3">
                      <label htmlFor="estatus_evento" className="form-label">
                        Estatus del Evento <span className="text-danger">*</span>
                      </label>
                      <select
                        className={`form-select ${formErrors.estatus_evento ? 'is-invalid' : ''}`}
                        id="estatus_evento"
                        name="estatus_evento"
                      ...
                      {formErrors.estatus_evento && (
                        <div className="invalid-feedback">{formErrors.estatus_evento}</div>
                      )}
                    </div>
                  </div>
                </div>
      ...
      {showDetailsModal && selectedEvent && (
        <div className="modal fade show d-block event-details-modal" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
              ...
              <div className="modal-body">
                <div className="event-info-grid">
                  <div className="event-info-item">
                    ...
                    <p><strong>Nombre:</strong> {selectedEvent.nombre_evento}</p>
                    <p><strong>Sede:</strong> {selectedEvent.sede_evento}</p>
                    <p><strong>Estatus:</strong> 
                      <span className={`badge bg-${getStatusBadgeColor(selectedEvent.estatus_evento)} ms-2`}>
                  ...
                  <div className="event-info-item">
                    ...
                    <p><strong>Inicio:</strong> {formatDateTime(selectedEvent.inicio_evento)}</p>
                    <p><strong>Fin:</strong> {formatDateTime(selectedEvent.fin_evento)}</p>
                    <p><strong>Límite de Participantes:</strong> {selectedEvent.lim_de_participantes_evento}</p>
                    <p><strong>Equipos Registrados:</strong> {selectedEvent.equipos?.length || 0}</p>
                  </div>
                </div>
                    ...
                    {selectedEvent.equipos.map((team) => (
                      <div key={team.id} className="team-card">
                        <div className="team-header">
                          <span className="team-name">{team.nombre_equipo}</span>
                          <span className={`badge bg-${team.estatus_del_equipo === 'activo' ? 'success' : 'secondary'} team-status`}>
                            {team.estatus_del_equipo === 'activo' ? 'Activo' : 'Inactivo'}
                          </span>
                        </div>
                        <div className="team-details">
                          <p><strong>Entidad Federativa:</strong> {team.entidad_federativa}</p>
                          <p><strong>Anfitrión:</strong> {team.nombre_anfitrion}</p>
                          <p><strong>Teléfono:</strong> {team.telefono_anfitrion}</p>
                          <p><strong>Correo:</strong> {team.correo_anfitrion}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="event-info-item">
                  ...
                  <p><strong>Creado:</strong> {formatDateTime(selectedEvent.created_at)}</p>
                  <p><strong>Actualizado:</strong> {formatDateTime(selectedEvent.updated_at)}</p>
                </div>
              </div>
              <div className="modal-footer">
                <button 
